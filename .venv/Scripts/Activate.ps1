...
# Define the new Get-VenvTerminalWidth function
function global:Get-VenvTerminalWidth {
    param()

    # Get the terminal width
    $terminalWidth = Get-TerminalWidth

    # If the virtual environment prompt prefix exists, subtract its length
    if ($global:_PYTHON_VENV_PROMPT_PREFIX) {
        $terminalWidth -= $global:_PYTHON_VENV_PROMPT_PREFIX.Length + 3 # +3 for "()" and space
    }
...
if (-not $Env:VIRTUAL_ENV_DISABLE_PROMPT) {
    ...
    function global:prompt {
        Write-Host -ForegroundColor Green "($_PYTHON_VENV_PROMPT_PREFIX) "
        # Execute the original prompt script block
        _OLD_VIRTUAL_PROMPT
    }
